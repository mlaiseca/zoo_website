...
class USERZOO
{
    ...
    public function login($uname,$upass)
    {
        try
        {
        $result=$this->db->prepare("SELECT customerid as UserID, customerpassword as Password FROM customer WHERE customerEmail=:uname UNION Select staffID as UserID, staffpassword as Password from Staff Where staffEmail=:uname LIMIT 1");
    ...
    public function showMenu($level)
    {
        ...
        $result=$this->db->prepare("SELECT UserTypeID FROM customer WHERE customerID=:uid UNION select UserTypeID  from Staff where staffID=:uid LIMIT 1");
        ...
        elseif($level==1 && $userLevel==1)
        {
            //print("customer");
            $sql="Select * from Menu Where menuSecurity<=$userLevel and menuText not in('Login') order by menuParent,menuSort ASC";
        }
        else
        {
            //print("other");
            $sql="Select * from Menu Where menuSecurity<=$userLevel and menuText not in ('Login','View Cart','Gift Shop','Events','Gallery') order by menuParent,menuSort ASC";
        }
        //$sql="Select * from Menu Where menuSecurity<=$userLevel order by menuParent,menuSort ASC";
        $querystmt=$this->db->prepare($sql);
        $querystmt->execute();
        ...
        if(!empty($menu))
        {
            ...
            foreach($menu as $page=>$link)
            {
                $name=$link['text'];
                $linkUrl=$link['link'];
                //if($name=='Home'|| $name=='Logout'||$name=='Login')
                if($linkUrl!="")
                {
