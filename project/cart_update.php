...
if(isset($_POST["type"]) && $_POST["type"]=='add' && $_POST["product_qty"]>0)
{
    ...
    $sql="Select itemDescription,itemPrice from item where itemID=$code";
    $qrystmt=$DB_con->prepare($sql);
    $qrystmt->execute();
    
    while($row=$qrystmt->fetchObject()){
        
        //fetch product name, price from db and add to new_product array
        $new_product["product_name"] = $row->itemDescription; 
        $new_product["product_price"] = $row->itemPrice;
